...
在创建组件包装器时，我们可以扩展原始组件的类型。这样封装器就会自动扩展原始组件的所有 prop，也可以避免在透传组件增加不必要的类型定义
...
### 提取 typescript 类型定义信息到 types.ts 文件中

根据组件复杂度决定是否单独提取出来（代码行数、type 行数）

### 分享资料汇总

[React Hooks 分享](https://s.tal.com/a/IXgcte)
[业务中 Hooks 分析](https://s.tal.com/a/Y6DgmK)
[React 中 state 管理](https://s.tal.com/a/vqKR9k)
